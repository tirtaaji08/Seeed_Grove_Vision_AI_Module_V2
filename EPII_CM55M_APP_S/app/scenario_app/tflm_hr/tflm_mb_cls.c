...
// #endif
	...
	if (max30102_init() == 0) {
        ...
        xprintf("MAX30102 init FAILED!\n");
	}
		cv_mb_cls_init(true, true, MOBILENET_CLASSIFICATION_FLASH_ADDR);

	    while (1) {
        if (max30102_available_samples(&avail) == 0 && avail > 0) {
            if (max30102_read_hr_sample(&raw_red) == 0) {
                // DEBUG: xprintf ("raw: %lu\n", raw_red);
                if (sample_count == 0) {
                   // sample1 = raw_red;       // simpan sampel pertama
                    sample_count++;
                } else {
                    // Kirim 2 sampel (sample1 dan raw_red) ke buffer preprocessing
					xprintf("R:%lu\n", raw_red); 
                    //ppg_add_samples_64hz(sample1, raw_red);
					ppg_add_sample_100hz(raw_red);
		...
		int ret = cv_mb_cls_run(&predicted_bpm);
